        ...
        <div class="max-w-md mx-auto sm:px-6 lg:px-8 bg-white shadow-md rounded-md">
            <form method="POST" action="{{ route('projects.update', $project->id) }}" class="p-4" enctype="multipart/form-data">
                ...
                <div class="mb-2">
                    ...
                    <textarea id="projectDescription" class="block mt-1 w-full resize-none" name="projectDescription" required autofocus rows="10" >{{ old('projectDescription', $project->projectDescription) }}</textarea>
                    <x-input-error :messages="$errors->get('projectDescription')" class="mt-2" />
                </div>

                <div>
                    <x-input-label for="skills" :value="__('Skills')" />
                    <select id="skills" name="skills" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 mb-2">
